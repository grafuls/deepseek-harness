...
- **The browser face** — [dsh-client-ui-auth](../../../../packages/client/ui-auth/README.md) covers the app with a sign-in card while the browser holds no session cookie; [dsh-client-ui-collab](../../../../packages/client/ui-collab/README.md) lists, creates, invites to, and administers workspaces from a sidebar foot trigger and an overlay panel, over the same shared `/api` RPC envelope the rest of the GUI uses. Both compose out via `slots.inject`, so without the collab overlay rows nothing mounts and a non-collab install renders unchanged.
