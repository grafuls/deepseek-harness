...
One process serves many users but keeps a single shared instance plane for the browser: one session cookie per browser, and a workspace is scoped to its data directory, not to its own login or sessions. The host plane serves each mounted collab workspace and the sessions bound inside it only to that workspace's members: the collab assembly stages a per-principal membership gate that the Host proxy consults on every request and stream, so a non-member sees no collab workspace in the standard list and a Host call into a hidden collab directory is refused with the host-owned error `workspace-forbidden` ([membership-scoped Host plane](2026-08-28-collab-host-plane-membership-scoping.md)). The gate covers every `/api` RPC and WebSocket upgrade, which is where the real authority lives; the GUI layers (ui-auth then ui-collab) only ever fail open and add wait-for-ready probes on top. The four exact auth routes deliberately bypass the RPC trust fence and are reachable on any host, which is safe only for loopback-first localhost deployments — recorded as a known tradeoff rather than silently "protected". The OAuth redirect origin is now derived per sign-in from the request when no origin is pinned, so a loopback-free remote deployment needs no origin configuration ([request-derived redirect origin](2026-08-29-collab-oauth-request-derived-redirect-origin.md)). RBAC lives in its own package with a typed permission matrix, so the per-workspace policy is testable without the full GUI. The whole feature's durability is pinned by a REAL-composition test that boots the `web-collab` profile through the Loader against a fake OIDC gateway; that composition also mounts the real Host workspace registry (memory domain store), and one case asserts that two members opening the same collab workspace resolve to the same real Workspace whose canonical path is the collab data directory.
