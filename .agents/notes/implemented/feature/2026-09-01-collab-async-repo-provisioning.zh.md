...
- 服务器在创建时递归创建克隆根目录（`cloneDir` 或默认 collab 布局的 `workspaces` 目录），因为 `git clone` 只创建叶子目标。无法创建的配置目录会让 create 立即以 `collab-bad-request` 应答——错误配置在最早已可解析的点响亮失败，而不是在注定失败的克隆之后移除工作区。
