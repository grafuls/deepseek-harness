...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default and, for a local launch, opens that canonical host URL only after the complete Loader tree settles. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. `--host 0.0.0.0` opts into all-interfaces serving for a trusted network: the runtime derives the machine's LAN IPv4 literals into the `/api` trust fence and prints a LAN URL beside the canonical loopback one. The Web carrier provides no authentication layer unless the deployment registers the [optional request authenticator](../../../packages/client/connection/README.md); privileged configuration and credential methods remain loopback-pinned. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
