...
- **One process session plane, per-workspace session binding** — the browser keeps one session cookie and the process hosts one global session plane, so a collab workspace hosts no login or session list of its own; opening a workspace mounts it as a real Host workspace, and the sessions a member starts inside it are bound to that workspace's shared `$DSH_HOME/collab/workspaces/<wsId>` data directory.
