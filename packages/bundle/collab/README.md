...
- **Live session plane is shared, not per-workspace** — the process's session plane stays global; per-workspace isolation is the durable `$DSH_HOME/collab/workspaces/<wsId>` data boundary.
