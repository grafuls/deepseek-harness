...
- **collab 工作区不会进入标准 Workspaces 列表** —— 本地浏览区会把 collab 来源的工作区及其会话从分组列表、平铺列表与搜索中过滤掉，因此它们只出现在 collab 区块，由该区块承载它们的行操作：工作区行选项菜单（重命名、打开管理器或删除——重命名与删除对每个成员生效，并在 collab 网关处以管理员权限把关）、工作区重命名对话框，以及按会话的重命名/分叉/归档对话框。成员管理请在这块面板中进行。
