...
创建入口（管理器的虚线按钮与区块工具栏的添加工作区按钮）会弹出一个承载协作工作区创建详情的模态对话框：一个工作区名称输入框和一个可选的「GitHub 仓库地址」输入框。填写地址会请求 collab API 把该仓库克隆到本地目录并把克隆结果作为工作区数据打开，而不是创建仅命名的（空）工作区。克隆目标在 `collab` 设置命名空间的 `cloneDir` 偏好被设置时取该值，否则取 collab 数据根下的 `workspaces` 目录；目标目录名为「仓库名-生成的工作区 id」，方便管理员一眼看出克隆源自哪个仓库。克隆在服务器端后台进行，因此创建请求立即应答、成功即关闭对话框；克隆尚未完成期间，工作区行显示「克隆中…」徽标，侧边栏区块中的该行同样显示徽标并禁用「新建会话」，打开该工作区会被以「仍在克隆中」横幅拒绝。后台克隆失败会静默移除该工作区，因此失败的仓库初始化不会留下任何注册记录。克隆落定后，在克隆内启动的会话各自运行在自己的分支上（服务器在会话创建时切换共享树），分支名为 `<workspace>-<session>`；浏览器在本地镜像同一套派生逻辑，以便命名这条线并路由会话的 git 动词，而无需额外往返。分支是会话级事实，因此任何工作区表面都不再展示它——管理器列表行、区块的工作区行与悬停信息框都不显示分支、提交或未提交更改标记；仅命名（无仓库）工作区在任何地方都不提供分支。会话悬停信息框会命名其所在线（`Branch: <workspace>-<session>`），会话行菜单在重命名/分叉/归档旁边携带三个 git 动词：「下载补丁」抓取会话分支相对工作区主干的统一 diff 并保存为 `.patch` 文件，成员可在本地用 `git apply` 应用（该 diff 读取提交对象，因此无论共享克隆当前检出哪个会话都一样）；「推送分支」打开确认门控的对话框，先加载服务器端试运行预览（会把什么推到哪个基线），然后在成员明确确认时真正推送，并把结果连同 HTTPS origin 的对比与新建拉取请求链接保留在对话框里；「同步」则抓取 origin。推送通过 collab API 的 git 凭据在服务器端进行且绝不强制推送，远端提示被移动时会拒绝而非覆盖；仓库地址只是 UI 便利入口——强制的克隆与推送契约及访问门禁在 collab API 中。

会话的「同步」动词是成员推送自己那条线之外的协作拉取补充：它请求 collab API 把 origin 的当前状态抓进共享克隆，而不触碰检出或任何会话的分支（只动远端跟踪引用），并以列表上方的瞬态提示确认。「下载补丁」是只读动词：它解析会话分支相对工作区主干基线的 diff，并把补丁文本交给浏览器保存为文件，并以列表上方的瞬态提示确认或报告失败（控制器绝不触碰 DOM——下载由浏览器负责）。推送本身走匹配 origin 主机的服务器凭据，因此即使仓库是私有的，成员也能在不必自持令牌的情况下共享提交。
...
- **collab 工作区只出现在「工作区」区块** —— 侧边栏浏览区已被移除，因此该区块是侧边栏唯一的工作区界面，直接承载 collab 的行操作：工作区行选项菜单（重命名、打开管理器或删除——重命名与删除对每个成员生效，并在 collab 网关处以管理员权限把关）、工作区重命名对话框，以及按会话的行菜单（重命名、分叉、归档、下载补丁、推送分支、同步）与其推送对话框。成员管理请在这块面板中进行。
...
- **删除工作区会移除记录与已落定克隆** —— `delete` 在面板中确认后，服务端注销工作区并删除仓库后端工作区所依托的克隆目录（仓库的用户副本；任何已推送的分支仍留在远端）。仅命名工作区的保留数据目录仍留在原地，与服务端删除语义一致。
