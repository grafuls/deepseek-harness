 ...
 * the transport: every browser follows the durable Host section through the
 * settings scope (scopes are host-backed on every origin now), and the server
 * answers each write, so success is judged against the state the write left
 * behind.
 */
...
/** Coordinates durable Host acknowledgement through the settings scope. */
export class WelcomeNoticeStore {
  ...
  /**
   * @param scope - the welcome settings namespace scope (host-backed on every
   * origin; the server answers each write).
   */
  ...
  /**
   * Persist this copy version through the scope. Success is judged against the
   * state the write left behind, so a refused or failed write reports false
   * after its recovery read settles.
   * @returns true when the acknowledgement persisted.
   */
