...
describe('ui-theme apply', () => {
  ...
  it('loads Host settings at boot, refreshes its namespace, and goes host-backed on remote browsers', async () => {
    const b = await bench()
    ...
    // A remote browser is host-backed too: the mirror reads and the preference
    // write both cross the wire, and the server decides whether they land.
