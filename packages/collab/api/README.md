...
| `collab/workspace.patch` | the unified diff of one branch against the workspace's mainline base as a `git apply`-compatible patch, with a suggested filename; read-only and member-gated |
...
`collab/workspace.push` moves one branch to the clone's origin; the repository row of the Collaborative Workspaces panel offers it (preview via a server dry run, then the member's own confirmation) as described above. `collab/workspace.fetch` syncs a settled clone with the origin without touching the checkout: it fetches remote-tracking refs and prunes stale ones, never the working tree or the session's current branch, so a member pulls the latest upstream into the shared clone without disturbing any session's line. It refuses a name-only or still-cloning record (`collab-not-a-repository`) and folds git failures back as `collab-bad-request`. `collab/workspace.patch` produces the unified diff of one branch against the workspace's mainline base — the `refs/remotes/origin/HEAD` default branch — as a patch a member can save and apply locally with `git apply`. It roots the diff at the branch's mainline merge-base: for a branch that is not the clone's current checkout it reads only commit objects, so it is deterministic and never includes another session's uncommitted working-tree edits, while a branch that is the checkout also carries that session's uncommitted edits (the working tree diffed against the merge-base). It needs no confirmation (a read-only, local diff) and no credential. It defaults to the checkout's current branch when no explicit `branch` is given, refuses a name-only or still-cloning record (`collab-not-a-repository`), and folds a detached checkout, an unknown mainline, or a git failure back as `collab-bad-request`. Pushing and fetching share the pinned server credential through a temporary `GIT_CONFIG_GLOBAL` whose authorization header carries the token only to the matching origin host; the token never reaches the browser, a wire URL, or the repository's own config (a patch is computed entirely locally and never needs it).
...
`collab/workspace.delete` now also removes the clone directory backing a repository-backed workspace (recursive and forced), so deleting a workspace closes its working tree instead of leaving roots behind; a clone that resists removal still completes the record deletion and only warns. A mounted collab workspace's Host registration is unregistered too, so a deleted workspace stops appearing in `workspace.list` (the hero workspace picker and sidebar) instead of leaking a record behind the collab Workspaces section.
