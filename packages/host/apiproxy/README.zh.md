...
当组合了多用户覆盖层时，工作区与会话的可见性按主体划分。网关在每次请求以及每条 `host()`/`mux()` 流打开时，都会带着连接主体读取可选的 `collabWorkspaceAccess` 服务——该服务仅由 `@deepseek-ai/dsh-collab-api` 提供：位于 `<collabRoot>/workspaces/<workspaceId>/…` 以内的路径，仅当该主体是该 collab 工作区的成员时才被列出、推送与可用，非成员指向此类目录的调用（工作区 create/rename/delete/insert、按 `workspaceId` 或 cwd 的 `session.create`、`history`、`fork`）会被携带工作区 id 的 host 自有错误 `workspace-forbidden` 拒绝，而普通 Host 工作区对每个已认证调用者依旧可见。门在每次决策时从实时服务存储读取，因此未组合覆盖层——或未解析出主体——的组合会让该平面逐字节不变。
...
- **两条 Host 实时回显可携带隐藏会话 id 但不携带会话内容**：在 collab 覆盖层下，`host()` 的归档会话回显与 `mux()` 的任务/队列/问题基线仍是进程全局的，因此可能携带调用者看不见的 collab 会话的工作区 id、会话 id 或任务状态；它们不携带任何会话内容，且枚举表面（`workspace.list`、`session.list`/`search`、`history`、`fork`）已被完全划定作用域。
