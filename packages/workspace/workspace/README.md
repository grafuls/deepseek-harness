...
- `ctx.workspaceRegistry.create(path, title?, collabWorkspaceId?)` — canonicalizes `path` via `fs.realpath`, rejects a nonexistent or non-directory path, creates at most one record per canonical path, and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title. A `collabWorkspaceId` marks the record collab-origin: a repeated mount re-stamps the marker onto an unmarked or recreated record so every member resolves the same collab owned workspace.
