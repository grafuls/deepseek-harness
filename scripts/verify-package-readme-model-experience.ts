...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-auth': { kind: 'none', reason: 'Browser-side presentation gate over the collab session verdict; it registers no model-facing behavior and the collab API gateway owns the enforced auth fence.' },
  'packages/client/ui-collab': { kind: 'none', reason: 'Browser-side workspaces manager over the collab RPC channel; it registers no model-facing behavior and the collab API gateway owns the auth-gated /api surface.' },
  ...
  'packages/bundle/collab': { kind: 'none', reason: 'The collab bundle is a patch-list carrier; each inserted collab service owns its model-facing behavior and the collab API gateway owns the auth-gated /api surface.' },
  ...
  'packages/collab/rbac': { kind: 'none', reason: 'The role/permission policy is a pure decision surface registering nothing model-facing; enforcement call sites own any model-visible effect.' },
  'packages/collab/users': { kind: 'none', reason: 'The user registry stores Google-identity accounts and registers nothing model-facing; collab surface consumers own any model-visible effect.' },
  'packages/collab/workspaces': { kind: 'none', reason: 'The workspace registry stores membership and invitations and registers nothing model-facing; collab surface consumers own any model-visible effect.' },
  'packages/collab/auth': { kind: 'none', reason: 'The auth service signs stateless session cookies and obtains identity, registering nothing model-facing; collab surface consumers own any model-visible effect.' },
  'packages/collab/api': { kind: 'none', reason: 'The API assembly forwards gate identity and collab service responses over RPC, registering nothing model-facing; any model request flows through the harness session surface it authorizes.' },
